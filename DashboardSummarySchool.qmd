...
dash.graph.da <- function(df, dist, school) {
...
**Question: Do we want to show the district DA table, or do we want to show the school dashboard info, without regard to those identified for DA?**
...
dash.graph.da(dash.mry.da,dist.name, school.name) 
...
p <- indicator.bar.school(df = dash.mry.da, dist = dist.name, school =  school.name , indie =   i)+
...
** Should these reflect the distict numbers needed to escape DA or only for the groups at the school that would have qualified to be at the higher status/color? **
